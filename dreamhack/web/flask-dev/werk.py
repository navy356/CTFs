# -*- coding: utf-8 -*- """ werkzeug.debug ~~~~~~~~~~~~~~ WSGI application traceback debugger. 
# :copyright: 2007 Pallets :license: BSD-3-Clause """ 
import getpass
import hashlib
import json
import mimetypes
...
def _generate():
    ...
    for filename in "/etc/machine-id", "/proc/sys/kernel/random/boot_id": 
        ...
        try:
            ...
            if linux:
                return linux 
        # On OS X, use ioreg to get the computer's serial number. try:
        ...
        try:
            ...
            if wr is not None:
                try: 
                    with wr.OpenKey( wr.HKEY_LOCAL_MACHINE, "SOFTWARE\\Microsoft\\Cryptography", 0, wr.KEY_READ | wr.KEY_WOW64_64KEY, ) as rk:
...
class _ConsoleFrame(object):
    ...
    def get_pin_and_cookie_name(app):
        """Given an application object this returns a semi-stable 9 digit pin code and a random key.
        The hope is that this is stable between restarts to not make debugging particularly
        frustrating. If the pin was forcefully disabled this returns `None`.
        ...
        # This information is here to make it harder for an attacker to # guess the cookie name.
        # They are unlikely to be contained anywhere # within the unauthenticated debug page.
