...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (min > 0) {
            this.f70mX = new float[min];
            this.f71mY = new float[min];
            float[] fArr = new float[2];
            for (int i2 = 0; i2 < min; i2++) {
                pathMeasure.getPosTan((((float) i2) * length) / ((float) (min - 1)), fArr, (float[]) null);
                this.f70mX[i2] = fArr[0];
                this.f71mY[i2] = fArr[1];
            }
            if (((double) Math.abs(this.f70mX[0])) <= 1.0E-5d && ((double) Math.abs(this.f71mY[0])) <= 1.0E-5d) {
                int i3 = min - 1;
                if (((double) Math.abs(this.f70mX[i3] - 1.0f)) <= 1.0E-5d && ((double) Math.abs(this.f71mY[i3] - 1.0f)) <= 1.0E-5d) {
                    float f = 0.0f;
                    int i4 = 0;
                    while (i < min) {
                        float[] fArr2 = this.f70mX;
                        int i5 = i4 + 1;
                        float f2 = fArr2[i4];
                        if (f2 >= f) {
                            fArr2[i] = f2;
            ...
            sb.append(this.f70mX[0]);
            sb.append(",");
            sb.append(this.f71mY[0]);
            sb.append(" end:");
            int i6 = min - 1;
            sb.append(this.f70mX[i6]);
            sb.append(",");
            sb.append(this.f71mY[i6]);
