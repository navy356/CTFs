...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() {
        public int compare(Snake snake, Snake snake2) {
            int i = snake.f61x - snake2.f61x;
            return i == 0 ? snake.f62y - snake2.f62y : i;
        }
    };
    ...
    public static DiffResult calculateDiff(Callback callback, boolean z) {
        ...
        while (!arrayList2.isEmpty()) {
            ...
            if (diffPartial != null) {
                ...
                diffPartial.f61x += range.oldListStart;
                diffPartial.f62y += range.newListStart;
                Range range2 = arrayList3.isEmpty() ? new Range() : (Range) arrayList3.remove(arrayList3.size() - 1);
                range2.oldListStart = range.oldListStart;
                range2.newListStart = range.newListStart;
                if (diffPartial.reverse) {
                    range2.oldListEnd = diffPartial.f61x;
                    range2.newListEnd = diffPartial.f62y;
                } else if (diffPartial.removal) {
                    range2.oldListEnd = diffPartial.f61x - 1;
                    range2.newListEnd = diffPartial.f62y;
                } else {
                    range2.oldListEnd = diffPartial.f61x;
                    range2.newListEnd = diffPartial.f62y - 1;
                }
                arrayList2.add(range2);
                if (!diffPartial.reverse) {
                    range.oldListStart = diffPartial.f61x + diffPartial.size;
                    range.newListStart = diffPartial.f62y + diffPartial.size;
                } else if (diffPartial.removal) {
                    range.oldListStart = diffPartial.f61x + diffPartial.size + 1;
                    range.newListStart = diffPartial.f62y + diffPartial.size;
                } else {
                    range.oldListStart = diffPartial.f61x + diffPartial.size;
                    range.newListStart = diffPartial.f62y + diffPartial.size + 1;
                }
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (snake == null || snake.f61x != 0 || snake.f62y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int size = this.mSnakes.size() - 1; size >= 0; size--) {
                ...
                int i3 = snake.f61x + snake.size;
                int i4 = snake.f62y + snake.size;
                ...
                for (int i5 = 0; i5 < snake.size; i5++) {
                    int i6 = snake.f61x + i5;
                    int i7 = snake.f62y + i5;
        ...
        private boolean findMatchingItem(int i, int i2, int i3, boolean z) {
            ...
            while (i3 >= 0) {
                ...
                int i6 = snake.f61x + snake.size;
                int i7 = snake.f62y + snake.size;
                int i8 = 8;
                if (z) {
        ...
        public void dispatchUpdatesTo(ListUpdateCallback listUpdateCallback) {
            ...
            for (int size = this.mSnakes.size() - 1; size >= 0; size--) {
                Snake snake = this.mSnakes.get(size);
                int i3 = snake.size;
                int i4 = snake.f61x + i3;
                int i5 = snake.f62y + i3;
                if (i4 < i) {
                ...
                for (int i6 = i3 - 1; i6 >= 0; i6--) {
                    if ((this.mOldItemStatuses[snake.f61x + i6] & 31) == 2) {
                        batchingListUpdateCallback.onChanged(snake.f61x + i6, 1, this.mCallback.getChangePayload(snake.f61x + i6, snake.f62y + i6));
                    }
                }
                i = snake.f61x;
                i2 = snake.f62y;
