...
MEASURE_FUNCS = os.getenv('BACKEND_INTERNAL_MEASURE_FUNCS', False)
MEASURE_CHUNKS = os.getenv('BACKEND_INTERNAL_MEASURE_CHUNKS', False)
...
class RequestsHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if record.levelno > logging.INFO or \
            ...
            self.session.post(f'http://logger/backend_internal/{tag}', json={
                'logs': self.buf
            })
            self.buf = []
